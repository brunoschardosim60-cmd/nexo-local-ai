  ...
  try {
    ...
    if (url.pathname === '/chat') {
      ...
      if (attachments.some(item => ['image','screen','camera'].includes(item.type))) {
        ...
        const directVisualRequest=/\b(?:analise|descreva|o que (?:tem|há|ha|aparece)|identifique|leia|extraia)\b/i.test(input.question)&&!/\b(?:edite|altere|mude|crie|gere|compare com|corrija|execute)\b/i.test(input.question);
        if(directVisualRequest&&perceptual.observations.length){const content=perceptual.observations.map(item=>item.content).filter(Boolean).join('\n\n');audit('runtime_vision_direct','vision',true,`${perceptual.observations.length} observações`);return send(response,200,{ok:true,kind:'instant',route:'vision',content,model:'Nexo Vision',perception:{modalities:perceptual.route.modalities,observations:perceptual.observations.map(({modality,source,confidence})=>({modality,source,confidence}))}});}
