...
export function createOperationalCapabilitySnapshot({
  ...
  const capabilities = [
    ...
    item('workspace', 'usar Google Calendar, Sheets, Drive e Gmail', Number(health.mcp?.configured || 0) > 0 ? 'CONDITIONAL' : 'UNAVAILABLE', Number(health.mcp?.configured || 0) > 0 ? 'O servidor MCP local está configurado; cada conta ainda depende de OAuth e as escritas exigem confirmação.' : 'Nenhum servidor Google Workspace MCP está configurado.', ['OAuth da conta Google', 'confirmação para escritas']),
...
function requestedCapability(question, capabilities) {
  ...
  const terms = {
    ...
    workspace: /\b(?:gmail|google\s+(?:calendar|sheets|drive)|calendario|agenda|e-?mail)\b/,
