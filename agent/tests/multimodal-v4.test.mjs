...
test('fila de mídia persiste, executa e pode cancelar', async () => {
  ...
  try { const job = queue.enqueue('image', { prompt: 'teste' }); await new Promise(resolve => setTimeout(resolve, 30)); assert.equal(queue.get(job.id).status, 'completed'); assert.equal(queue.get(job.id).artifactId, 'generated-1'); assert.equal(queue.get(job.id).result.artifact.id, 'generated-1'); const pending = queue.enqueue('unknown', {}); await new Promise(resolve => setTimeout(resolve, 20)); assert.equal(queue.get(pending.id).status, 'failed'); } finally { database.db.close(); await rm(directory, { recursive: true, force: true }); }
});
