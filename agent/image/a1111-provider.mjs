...
export function createA1111ImageProvider({ baseUrl, fetchImpl = globalThis.fetch }) {
  ...
  async function generate(plan, { signal } = {}) {
    ...
    const response = await fetchImpl(`${baseUrl}/sdapi/v1/txt2img`, { method: 'POST', headers: { 'content-type': 'application/json' }, signal: signal || AbortSignal.timeout(600_000), body: JSON.stringify({ prompt: plan.prompt, negative_prompt: plan.negativePrompt, width: plan.width, height: plan.height, steps: plan.steps, cfg_scale: plan.cfgScale, sampler_name: plan.samplerName, scheduler: plan.scheduler, seed: plan.seed, batch_size: 1, n_iter: 1 }) });
  ...
  async function release() { try { await fetchImpl(`${baseUrl}/sdapi/v1/unload-checkpoint`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' }); } catch {} }
  return { id: 'automatic1111-forge', capabilities:['image-generation','text-to-image','image-to-image','inpainting','variations','upscale'], probe, generate, edit, release, health: () => ({ provider: 'Stable Diffusion WebUI/Forge/A1111-compatible', baseUrl, ...state, supports: ['text-to-image','image-to-image','inpainting','variations','upscale'] }) };
}
