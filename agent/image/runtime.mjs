...
export function createImageRuntime({ provider, artifacts, vision = null, enabled = true, autoRegeneration = false }) {
  async function generate(input, options = {}) {
    if (!enabled) throw new Error('Geração de imagem está desativada.'); const plan = planImagePrompt(input); const editing=plan.mode!=='text-to-image'||Boolean(plan.sourceImage);const generated = editing&&provider.edit?await provider.edit(plan,options):await provider.generate(plan, options);
