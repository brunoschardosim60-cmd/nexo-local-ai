...
export function createEventBus({ database, logger }) {
  ...
  async function publish(type, data = null, options = {}) {
    const event = database.addRuntimeEvent(type, data, options);
  ...
  return {
    ...
    health() { return { persistent: true, subscribers: [...subscribers.values()].reduce((total, handlers) => total + handlers.size, 0) }; },
  };
}
