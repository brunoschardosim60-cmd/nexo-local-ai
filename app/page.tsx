      ...
      <aside className="hidden min-h-0 border-l border-border bg-sidebar/60 2xl:block">
        <ScrollArea className="h-full"><div className="p-5">
          ...
          <div className="mt-6 space-y-2.5">{[
            ...
            { name: 'Browser Agent', detail: agentHealth?.agent?.capabilities?.browser?.screenshots ? `Sessões + ${agentHealth.agent.capabilities.browser.engine}` : 'Navegação segura disponível', active: !!agentHealth?.agent?.capabilities?.browser?.available, icon: Globe2 },
          ...
          <div className="mt-6 rounded-2xl border border-primary/15 bg-primary/7 p-4"><p className="text-xs font-medium text-primary">Nexo Core {agentHealth?.agent?.version || 'local'}</p><div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">{['Task Graph', 'Checkpoints', 'Tool contracts', 'Repository map', 'Context Engine', 'RAG local', 'Research Agent', 'Browser Agent', 'Skills locais', 'MCP stdio', 'Background jobs', 'Visual verifier', 'Multi-agent 4×'].map(capability => <span key={capability} className="rounded-lg bg-muted px-2 py-1.5">{capability}</span>)}</div></div>
        </div></ScrollArea>
      </aside>
    </div>
