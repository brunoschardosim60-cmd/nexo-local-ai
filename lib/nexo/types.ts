...
export type LocalAttachment = {
  type: 'image' | 'audio' | 'video' | 'screen' | 'camera';
  name: string;
  mimeType: string;
  dataUrl: string;
};
...
export type ChatMessage = {
  ...
  attachments?: Array<Omit<LocalAttachment, 'dataUrl'>>;
};
export type Chat = {
  id: string;
  title: string;
...
export type PersonalSettings = {
  proactivityLevel: 'OFF' | 'LOW' | 'NORMAL' | 'HIGH';
  notificationsEnabled: boolean;
  quietHours: { enabled: boolean; start: string; end: string };
  interruptionBudget: { maxPerDay: number; minMinutesBetween: number };
  dailyBriefEnabled: boolean;
  endOfDayReviewEnabled: boolean;
  learningHistoryEnabled: boolean;
  spacedRepetitionEnabled: boolean;
  tutorMode: 'GUIDE' | 'TEACH' | 'CHALLENGE' | 'EXAM';
  dontSpoil: boolean;
  focusMode: boolean;
};
...
export type PersonalDashboard = {
  today: {
    ...
    importantDeadlines: Array<
      (PersonalGoal | PersonalTask) & {
        risk: { level: string; reason: string; confidence: number };
      }
    >;
    unfinishedWork: PersonalTask[];
    recentChanges: RuntimeEvent[];
    recommendedFocus?: { taskId: string; title: string; reason: string } | null;
  };
...
export type NexoAction = {
  ...
  path: string;
  content?: string;
  template?: 'static-site' | 'node-api' | 'python-api' | 'ai-service';
  reason: string;
...
export type AgentTaskStep = {
  ...
  title: string;
  description: string;
  status: 'pending' | 'ready' | 'awaiting_approval' | 'completed' | 'failed';
  dependencies?: string[];
...
export type AgentTaskNode = AgentTaskStep & {
  taskId: string;
...
export type NexoCapability = {
  id: string;
  type: 'TOOL' | 'SKILL' | 'MCP' | 'PROVIDER' | 'WORKFLOW' | 'CONNECTOR';
  name: string;
  version: string;
  description: string;
  permissions: string[];
...
export type NexoWorkflow = {
  id: string;
  name: string;
  version: string;
  enabled: boolean;
  definition: {
    steps: Array<{ id: string; type: string; dependsOn?: string[] }>;
  };
  updatedAt: string;
};
...
export type AgentTask = {
  ...
  goal?: {
    objective: string;
    completionState: 'OPEN' | 'VERIFIED' | 'FAILED' | 'UNCERTAIN';
    constraints: string[];
    acceptanceCriteria: Array<{
      id: string;
      criterion: string;
      status: 'PASS' | 'FAIL' | 'UNCERTAIN' | 'NOT_CHECKED';
      evidence: string[];
    }>;
  ...
  workingMemory?: {
    ...
    evidence?: Array<{ tool: string; ok: boolean; step: string; at: string }>;
  };
  result?: {
...
export type AgentHealth = {
  ...
  network: {
    interfaces: Array<{ name: string; vpn: boolean }>;
    vpnDetected: boolean;
  };
  agent?: {
    runtime?: string;
    ...
    capabilities?: {
      ...
      continuity?: {
        engine: string;
        persistentHandoffs: boolean;
        projectAware: boolean;
      };
      personal?: Record<string, unknown>;
      personalWork?: Record<string, unknown>;
      study?: Record<string, unknown>;
      proactivity?: Record<string, unknown>;
      personalSearch?: Record<string, unknown>;
      ...
      mediaQueue?: Record<string, unknown>;
    };
  };
};
...
export type NexoMemory = {
  ...
  summary: string;
  scope: string;
  privacy: 'LOCAL_ONLY' | 'SHAREABLE' | 'RESTRICTED';
  status: 'ACTIVE' | 'UNCERTAIN' | 'SUPERSEDED' | 'FORGOTTEN' | 'DELETED';
  confidence: number;
  importance: number;
  source: string;
  topics: string[];
